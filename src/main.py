...
transport = Transportation(api_id)
...
for row_data in all_row_data:
    try:
        ...
        # data_admissao = datetime.strptime(row_data.data_admissao, "%Y-%m-%d").strftime("%Y-%m-%d")
        # data_demissao = datetime.strptime(row_data.data_demissao, "%Y-%m-%d").strftime("%Y-%m-%d")
        

                
        # Call edit_workflow method with the fetched record_id and row data
        transport.edit_workflow(
            EntityID                    = row_data.EntityID,  # Use the fetched record_id here
            nome                        = row_data.nome,
